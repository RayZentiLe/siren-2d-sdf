...
def sdf(model_output, gt):
    ...
    on_surface_mask = (gt_sdf == 0).float()
    off_surface_mask = (gt_sdf != 0).float()

    # Direct supervision on all points
    sdf_supervision = torch.abs(pred_sdf - gt_sdf)

    # Interpolation constraint only for points far from surface?
    inter_constraint = off_surface_mask * torch.exp(-1e2 * torch.abs(pred_sdf))

    # Normal constraint only on on-surface
    normal_constraint = on_surface_mask * (1 - F.cosine_similarity(gradient, gt_normals, dim=-1)[..., None])

    # Eikonal on all points
    grad_constraint = torch.abs(gradient.norm(dim=-1) - 1)

    return {
        'sdf_supervision': sdf_supervision.mean() * 1e3,
        'inter': inter_constraint.mean() * 1e2,  # Optional
